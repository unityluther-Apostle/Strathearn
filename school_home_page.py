...
def view_system_logs_content(logs_tab):
    with ui.tab_panel(logs_tab).classes('p-0 gap-4 flex flex-col'):
        with ui.card().classes('w-full p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10 flex flex-col gap-5'):
            log_columns = [
            ...
            all_log_records = []
            activity_table = ui.table(columns=log_columns, rows=[], row_key='id').classes('w-full shadow-none border border-emerald-900/10 rounded-2xl')
            ...
            def clear_all_logs():
                with ui.dialog() as confirm_dialog, ui.card().classes('p-6 gap-4 rounded-3xl'):
                    ui.label('Are you sure you want to delete all system activity logs?').classes('text-base font-bold text-slate-800')
                    ui.label('This action cannot be undone.').classes('text-xs text-slate-500')
                    with ui.row().classes('w-full justify-end gap-2'):
                        ...
                        ui.button('Delete All', on_click=execute_clear).props('color="red" unelevated')
                confirm_dialog.open()

            with ui.row().classes('w-full justify-between items-center flex-wrap gap-3'):
                ui.label('System Activity Logs & User Management').classes('text-lg font-bold text-slate-900 tracking-tight')

                with ui.row().classes('items-center gap-2.5'):
                    search_input = ui.input(placeholder='Search username or activity...').props('dense outlined clearable rounded-pill')
                    ui.button(icon='search', on_click=lambda: apply_log_filter(search_input.value)).props('dense color="primary" unelevated round')
                    ui.button('Refresh Logs', icon='refresh', on_click=refresh_activity_table).props('outline dense text-color="primary" rounded-pill')
                    ui.button('Clear All Logs', icon='delete_sweep', on_click=clear_all_logs).props('dense color="red-7" unelevated rounded-pill')

            with ui.row().classes('w-full gap-3 items-center bg-gradient-to-r from-emerald-50/50 to-teal-50/20 p-4 rounded-2xl border border-emerald-900/10'):
                ui.icon('admin_panel_settings', color='primary').classes('text-lg')
                ui.label('Quick User Actions:').classes('font-semibold text-sm text-slate-800')
                target_user_input = ui.input(placeholder='Enter username...').props('dense outlined bg-white rounded-pill').classes('w-52')
                ...
                ui.button('Force Logout', icon='logout', on_click=force_logout_user).props('dense color="orange" unelevated rounded-pill')
...
def open_edit_dialog(record):
    with ui.dialog() as edit_dialog, ui.card().classes('w-[420px] p-6 gap-4 rounded-3xl shadow-2xl'):
        ui.label(f"Edit Record: {record['Name']}").classes('text-lg font-bold text-slate-900')

        edit_name = ui.input('Name', value=record['Name']).classes('w-full').props('outlined rounded-pill dense')
        edit_payment_code = ui.input('Payment Code', value=record.get('PaymentCode', '')).classes('w-full').props('outlined rounded-pill dense')
        edit_class = ui.input('Class', value=record['Class']).classes('w-full').props('outlined rounded-pill dense')
        edit_year = ui.input('Year', value=record.get('Year', str(datetime.now().year))).classes('w-full').props('outlined rounded-pill dense')
        edit_term = ui.select(['Term I', 'Term II', 'Term III'], value=record.get('Term', 'Term I'), label='Term').classes('w-full').props('outlined rounded-pill dense')
        edit_maths = ui.number('Maths', value=record['Maths']).classes('w-full').props('outlined rounded-pill dense')
        edit_english = ui.number('English', value=record['English']).classes('w-full').props('outlined rounded-pill dense')
        edit_sst = ui.number('SST', value=record['SST']).classes('w-full').props('outlined rounded-pill dense')
        edit_science = ui.number('Science', value=record['Science']).classes('w-full').props('outlined rounded-pill dense')
        ...
        with ui.row().classes('w-full justify-end gap-2 mt-4'):
            ui.button('Cancel', on_click=edit_dialog.close).props('flat rounded-pill')
            ui.button('Save Changes', on_click=save_changes).props('unelevated color="primary" rounded-pill')
...
def init_chat_db():
    with sqlite3.connect(DB) as conn:
        ...
        ''')
        # Safely add created_at column if missing in pre-existing tables
        cursor = conn.cursor()
        ...
        if 'created_at' not in columns_info:
            try:
                ...
                # Backfill created_at with current time for legacy rows if needed
...
def staff_chat_content():
    chat_container = ui.column().classes(
        'w-full h-[450px] overflow-y-auto p-5 bg-slate-50/50 rounded-2xl gap-3 border border-emerald-900/10'
    )
    ...
    def clear_old_messages():
        try:
            with sqlite3.connect(DB) as conn:
                ...
                # Check columns to safely delete using created_at or fallback
                cursor.execute("PRAGMA table_info(staff_chat)")
                ...
                if 'created_at' in cols:
                    ...
                    cursor.execute('DELETE FROM staff_chat') # Fallback if column cannot be added
            ...
            try:
                ...
                if not messages:
                    with ui.column().classes(
                        ...
                        ui.icon('chat_bubble_outline', size='48px', color='#1b4d3e').classes('opacity-40 animate-pulse')
                    ...
                    for msg in messages:
                        ...
                        with ui.column().classes(f'w-full {align} gap-1'):
                            ...
                            bubble = (
                                'bg-[#1b4d3e] text-white rounded-br-sm shadow-sm'
                                if mine
                                else
                                'bg-white text-slate-700 border border-slate-200/80 rounded-bl-sm shadow-sm'
                            )
                            ...
                            with ui.card().classes(f'{bubble} max-w-[75%] px-4 py-2.5 rounded-2xl'):
                                ...
                                with ui.row().classes('justify-end items-center gap-1.5 mt-1'):
                                    ...
                                    if mine:
                                        ui.icon('done_all', size='13px').classes('text-emerald-100')
                                        ui.button(
                                            icon='delete_outline',
                                            on_click=lambda mid=msg['id']: delete_message(mid)
                                        ).props('flat dense round size="xs"').classes('text-white/70 hover:bg-white/10')
            except Exception as e:
                ui.label(f'Chat error: {e}').classes('text-red-500')

    with ui.column().classes(
        'w-full max-w-4xl mx-auto bg-white rounded-3xl shadow-md border border-emerald-900/10 overflow-hidden'
    ):
        with ui.row().classes(
            'w-full items-center justify-between bg-white px-6 py-4 border-b border-emerald-900/10'
        ):
            with ui.row().classes('items-center gap-3.5'):
                with ui.avatar().classes('bg-gradient-to-br from-[#1b4d3e] to-teal-800 text-white shadow-md'):
                    ui.icon('school')

                with ui.column().classes('gap-0'):
                    ui.label('Staff Chat').classes('font-bold text-slate-900 text-base')
                    ui.label(current_vibe).classes('text-xs text-emerald-700 font-medium')

            with ui.row().classes('items-center gap-2.5'):
                ui.badge('Online').classes('bg-emerald-50 text-emerald-800 text-xs font-semibold border border-emerald-200 px-3 py-1 rounded-full')
                ui.button('Clear 24h+', icon='history_toggle_off', on_click=clear_old_messages).props('outline dense color="red-7" rounded-pill size="sm"')
        ...
        with ui.row().classes(
            'w-full px-5 py-2.5 bg-slate-50/70 border-t border-emerald-900/10 gap-1 overflow-x-auto'
        ):
            for emoji in emojis:
                ...
                ui.button(emoji, on_click=add_emoji).props('flat dense size="sm"').classes('rounded-full hover:bg-emerald-100/50')

        with ui.row().classes('w-full items-center gap-3 p-4 bg-white border-t border-emerald-900/10'):
            msg_input = ui.input(placeholder='Write a message...').props('borderless dense').classes('flex-1 bg-slate-50 rounded-full px-6 py-1 border border-slate-200 focus-within:border-[#1b4d3e]')
            ...
            ui.button(icon='send', on_click=send_message).props('round unelevated').classes('bg-gradient-to-br from-[#1b4d3e] to-teal-800 text-white w-11 h-11 shadow-md hover:scale-105 transition-transform')

def edit_lower_record(record, on_save):
    with ui.dialog() as edit_dialog, ui.card().classes('w-[480px] p-6 gap-3 rounded-3xl shadow-2xl'):
        ui.label(f"Edit Lower Primary Record: {record.get('name', '')}").classes('text-lg font-bold text-slate-900')

        edit_name = ui.input('Pupil Name', value=record.get('name', '')).classes('w-full').props('outlined rounded-pill dense')
        edit_payment_code = ui.input('Payment Code', value=record.get('payment_code', '')).classes('w-full').props('outlined rounded-pill dense')
        edit_class = ui.input('Class', value=str(record.get('class_level', ''))).classes('w-full').props('outlined rounded-pill dense')
        current_year_str = str(datetime.now().year)
        edit_year = ui.input('Year', value=str(record.get('year', current_year_str))).classes('w-full').props('outlined rounded-pill dense')
        edit_term = ui.select(['Term I', 'Term II', 'Term III'], value=str(record.get('term', 'Term I')), label='Term').classes('w-full').props('outlined rounded-pill dense')
        
        edit_lit_i = ui.number('Lit I', value=record.get('literacy_i', 0)).classes('w-full').props('outlined rounded-pill dense')
        edit_lit_ii = ui.number('Lit II', value=record.get('literacy_ii', 0)).classes('w-full').props('outlined rounded-pill dense')
        edit_reading = ui.number('Reading', value=record.get('reading', 0)).classes('w-full').props('outlined rounded-pill dense')
        edit_luganda = ui.number('Luganda', value=record.get('luganda', 0)).classes('w-full').props('outlined rounded-pill dense')
        edit_maths = ui.number('Maths', value=record.get('mathematics', 0)).classes('w-full').props('outlined rounded-pill dense')
        edit_english = ui.number('English', value=record.get('english', 0)).classes('w-full').props('outlined rounded-pill dense')
        edit_sst = ui.number('S.S.T', value=record.get('social_studies', 0)).classes('w-full').props('outlined rounded-pill dense')
        edit_science = ui.number('Science', value=record.get('science', 0)).classes('w-full').props('outlined rounded-pill dense')
        edit_re = ui.number('R.E', value=record.get('re_religious_education', 0)).classes('w-full').props('outlined rounded-pill dense')
        ...
        with ui.row().classes('w-full justify-end gap-2 mt-4'):
            ui.button('Cancel', on_click=edit_dialog.close).props('flat rounded-pill')
            ui.button('Save Changes', on_click=save_lower_changes).props('unelevated color="primary" rounded-pill')
...
def view_lower_records_content(lower_primary_tab):
    with ui.tab_panel(lower_primary_tab).classes('p-0 gap-5 flex flex-col'):
        with ui.card().classes('w-full p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10 flex flex-col gap-5'):
            ...
            with ui.row().classes('w-full justify-between items-center flex-wrap gap-3'):
                ui.label('Lower Primary Academic Records').classes('text-lg font-bold text-slate-900 tracking-tight')
                ...
                with ui.row().classes('items-center gap-2.5 flex-wrap'):
                    ...
                    ).props('dense outlined rounded-pill').classes('w-32')

                    year_input = ui.input(
                        placeholder='Search Year...',
                        on_change=lambda: refresh_lower_table_data(class_select.value, year_input.value, term_select.value)
                    ).props('dense outlined rounded-pill clearable').classes('w-36')
                    ...
                    term_select = ui.select(
                        ...
                        on_change=lambda: refresh_lower_table_data(class_select.value, year_input.value, term_select.value)
                    ).props('dense outlined rounded-pill').classes('w-36')
                ...
                with ui.row().classes('items-center gap-2.5 flex-wrap'):
                    ...
                    search_input = ui.input(placeholder='Search pupil name...').props('dense outlined clearable rounded-pill')
                    ui.button(icon='search', on_click=lambda: apply_lower_filter(search_input.value)).props('dense color="primary" unelevated round')
                    ui.button('Print All', icon='print', on_click=download_all_lower_reports).props('flat dense color="primary" rounded-pill')
                    ui.button('Export CSV', icon='download', on_click=export_csv).props('flat dense color="primary" rounded-pill')
                    ui.button('Refresh', icon='refresh', on_click=lambda: refresh_lower_table_data(class_select.value, year_input.value, term_select.value)).props('outline dense text-color="primary" rounded-pill')
            ...
            all_lower_records = []
            lower_student_table = ui.table(columns=columns, rows=[], row_key='id').classes('w-full shadow-none border border-emerald-900/10 rounded-2xl')
            
            lower_student_table.add_slot('body-cell-_calc_grade', '''
                <q-td :props="props">
                    <q-badge :color="['D', 'E', 'F', 'F9'].includes(props.value) ? 'red' : 'green'" class="px-2.5 py-1 rounded-full font-semibold">
                        {{ props.value }}
                    </q-badge>
                </q-td>
            ''')
            ...
            lower_student_table.add_slot('body-cell-actions', '''
                <q-td :props="props">
                    <q-btn flat dense round icon="download" color="green-7" @click="$parent.$emit('download_lower_row', props.row)" class="hover:bg-emerald-50">
                        <q-tooltip>Download Report</q-tooltip>
                    </q-btn>
                    <q-btn flat dense round icon="edit" color="blue-7" @click="$parent.$emit('edit_lower_row', props.row)" class="hover:bg-blue-50">
                        <q-tooltip>Edit Record</q-tooltip>
                    </q-btn>
                    <q-btn flat dense round icon="delete" color="red-7" @click="$parent.$emit('delete_lower_row', props.row.id)" class="hover:bg-red-50">
            ...
            def refresh_lower_table_data(class_filter='All', year_filter='', term_filter='All'):
                ...
                try:
                    with sqlite3.connect(DB) as conn:
                        ...
                        # Handle column checking and migration for lower_primary_results table
                        cursor = conn.cursor()
                        ...
                        # Backfill null or empty year/term values
...
def home(client=None):
    ...
    ui.query('.nicegui-content').classes('flex flex-col items-center bg-gradient-to-br from-slate-50 via-teal-50/20 to-emerald-50/20 p-4 md:p-8')

    dashboard_data = get_dashboard_stats()

    with ui.column().classes('w-full max-w-[1360px] items-center gap-6'):
        with ui.card().classes('w-full p-4 bg-white/90 backdrop-blur-md shadow-lg rounded-3xl border border-emerald-900/10').tight():
            with ui.row().classes('w-full items-center justify-between px-4 py-2'):
                with ui.row().classes('items-center gap-4'):
                    ui.icon('school', color='#1b4d3e').classes('text-3xl p-3 bg-gradient-to-br from-emerald-100 to-teal-50 rounded-2xl shadow-sm')
                    with ui.column().classes('gap-0'):
                        ui.label('AIM Pre-School').classes('text-2xl font-black tracking-tight text-slate-900')
                        ui.label('Academic Information Management').classes('text-[11px] font-semibold text-emerald-800/70 tracking-wide uppercase')
                    ui.add_head_html('''
                            <style>
                ...
                with ui.row().classes('items-center gap-4'):
                    with ui.row().classes('items-center gap-2 bg-emerald-50/60 px-3.5 py-2 rounded-2xl border border-emerald-900/10 shadow-inner'):
                        ui.icon('timer', size='16px', color='primary')
                        countdown_label = ui.label('Auto-logout in: 30:00').classes('text-xs font-bold text-slate-700')

                    global unread_notifications_badge, notifications_menu
                    with ui.row().classes('items-center gap-2 relative'):
                        with ui.button(icon='notifications').props('flat round dense').classes('text-slate-700 hover:bg-emerald-50'):
                        ...
                        with ui.menu() as notifications_menu:
                            with ui.card().classes('w-80 p-4 shadow-2xl rounded-3xl border border-emerald-900/10'):
                                ui.label('Live System Notifications').classes('text-xs font-bold text-slate-800 mb-2 border-b border-slate-100 pb-2')
                                                    ...
                                                    for log in recent_logs:
                                                        with ui.row().classes('w-full justify-between items-start py-2 border-b border-slate-50 last:border-none'):
                                                            with ui.column().classes('gap-0 flex-1'):
                                                                ...
                                                                ui.label(log['timestamp']).classes('text-[9px] text-slate-400 font-semibold')
                                    except Exception:
                                        pass
                ...
                with ui.tabs().classes(
                        'w-full bg-slate-100/70 p-2 rounded-2xl border border-emerald-900/10 big-tabs'
                    ) as nav_tabs:
                    nav_tabs.classes('items-center justify-center')

                    home_tab = ui.tab('Home Dashboard', icon='dashboard').classes('rounded-xl transition-all duration-300 text-slate-700 data-[selected]:bg-[#1b4d3e] data-[selected]:text-white data-[selected]:shadow-md')
                    teachers_tab = ui.tab('Entry Manager', icon='edit_note').classes('rounded-xl transition-all duration-300 text-slate-700 data-[selected]:bg-[#1b4d3e] data-[selected]:text-white data-[selected]:shadow-md')
                    Upper_primary_tab = ui.tab('Upper Primary Records', icon='assignment').classes('rounded-xl transition-all duration-300 text-slate-700 data-[selected]:bg-[#1b4d3e] data-[selected]:text-white data-[selected]:shadow-md')
                    lower_primary_tab = ui.tab('Lower Primary Records', icon='menu_book').classes('rounded-xl transition-all duration-300 text-slate-700 data-[selected]:bg-[#1b4d3e] data-[selected]:text-white data-[selected]:shadow-md')
                    staff_chat_tab = ui.tab('Staff Chat', icon='forum').classes('rounded-xl transition-all duration-300 text-slate-700 data-[selected]:bg-[#1b4d3e] data-[selected]:text-white data-[selected]:shadow-md')
                    logs_tab = ui.tab('System Logs', icon='history').classes('rounded-xl transition-all duration-300 text-slate-700 data-[selected]:bg-[#1b4d3e] data-[selected]:text-white data-[selected]:shadow-md')
                    logout_tab = ui.tab('Log Out', icon='logout').classes('rounded-xl transition-all duration-300 text-red-600 data-[selected]:bg-red-50 data-[selected]:text-red-700 data-[selected]:shadow-md')
        ...
        with ui.tab_panels(nav_tabs, value=starting_tab).classes('w-full text-xl bg-transparent'):
            with ui.tab_panel(home_tab).classes('p-0 gap-6 flex flex-col'):
                with ui.card().classes('w-full p-8 bg-white shadow-md rounded-3xl border border-emerald-900/10'):
                    with ui.row().classes('w-full items-center no-wrap gap-6'):
                        ui.image('badge.jpeg') \
                            .classes('w-24 h-24 md:w-28 md:h-28 rounded-3xl shadow-md object-cover border-2 border-emerald-900/10 flex-shrink-0')

                        with ui.column().classes('flex-1 justify-center gap-1.5'):
                            ui.label("Administrator Dashboard") \
                                .classes('text-2xl md:text-3xl font-black tracking-tight text-slate-900').style('color: #1b4d3e !important;')

                            ui.label(f"System Status: Operational • {datetime.now().strftime('%A, %B %d, %Y')}") \
                                .classes('text-emerald-800/80 text-xs font-bold tracking-wide uppercase')
                    ...
                    with ui.row().classes('w-full gap-4 items-stretch mt-6'):
                        with ui.card().classes('flex-1 p-5 bg-gradient-to-br from-white via-emerald-50/20 to-teal-50/30 shadow-md rounded-3xl border border-emerald-900/10 cursor-pointer hover:border-[#1b4d3e] hover:shadow-lg transition-all group') as btn_card1:
                            with ui.row().classes('items-center gap-4'):
                                ui.icon('person_add', color='primary').classes('text-2xl p-3.5 bg-emerald-100/70 rounded-2xl group-hover:scale-110 transition-transform shadow-sm')
                                with ui.column().classes('gap-0'):
                                    ui.label('Add New Scores').classes('text-sm font-bold text-slate-900')
                                    ui.label('Open marks entry form').classes('text-xs text-slate-500 font-medium')
                            btn_card1.on('click', lambda: nav_tabs.set_value(teachers_tab))

                        with ui.card().classes('flex-1 p-5 bg-gradient-to-br from-white via-emerald-50/20 to-teal-50/30 shadow-md rounded-3xl border border-emerald-900/10 cursor-pointer hover:border-[#1b4d3e] hover:shadow-lg transition-all group') as btn_card2:
                            with ui.row().classes('items-center gap-4'):
                                ui.icon('print', color='primary').classes('text-2xl p-3.5 bg-emerald-100/70 rounded-2xl group-hover:scale-110 transition-transform shadow-sm')
                                with ui.column().classes('gap-0'):
                                    ui.label('Bulk Print Reports').classes('text-sm font-bold text-slate-900')
                                    ui.label('Export class report sheets').classes('text-xs text-slate-500 font-medium')
                            btn_card2.on('click', download_all_reports)

                        with ui.card().classes('flex-1 p-5 bg-gradient-to-br from-white via-emerald-50/20 to-teal-50/30 shadow-md rounded-3xl border border-emerald-900/10 cursor-pointer hover:border-[#1b4d3e] hover:shadow-lg transition-all group') as btn_card3:
                            with ui.row().classes('items-center gap-4'):
                                ui.icon('forum', color='primary').classes('text-2xl p-3.5 bg-emerald-100/70 rounded-2xl group-hover:scale-110 transition-transform shadow-sm')
                                with ui.column().classes('gap-0'):
                                    ui.label('Staff Chat Hub').classes('text-sm font-bold text-slate-900')
                                    ui.label('Check teacher discussions').classes('text-xs text-slate-500 font-medium')
                            btn_card3.on('click', lambda: nav_tabs.set_value(staff_chat_tab))

                    with ui.row().classes('w-full gap-5 items-stretch mt-4'):
                        with ui.card().classes('flex-1 p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10 hover:shadow-lg transition-shadow'):
                            with ui.row().classes('items-center gap-2.5'):
                                ui.icon('group', color='primary').classes('text-xl p-2 bg-emerald-50 rounded-xl')
                                ui.label('Total Enrollment').classes('text-xs font-bold uppercase text-emerald-800/80 tracking-wider')
                            ui.label(str(dashboard_data.get('total_students', 0))).classes('text-4xl font-black text-slate-900 mt-3')
                            ui.label('Active Student Records').classes('text-xs font-semibold text-slate-400 mt-1')

                        with ui.card().classes('flex-1 p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10 hover:shadow-lg transition-shadow'):
                            with ui.row().classes('items-center gap-2.5'):
                                ui.icon('school', color='primary').classes('text-xl p-2 bg-emerald-50 rounded-xl')
                                ui.label('Primary Classes').classes('text-xs font-bold uppercase text-emerald-800/80 tracking-wider')
                            ui.label('P1 - P7').classes('text-4xl font-black text-slate-900 mt-3')
                            ui.label('Controlled classroom environments.').classes('text-xs font-semibold text-slate-400 mt-1')

                        with ui.card().classes('flex-1 p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10 hover:shadow-lg transition-shadow'):
                            with ui.row().classes('items-center gap-2.5'):
                                ui.icon('trending_up', color='primary').classes('text-xl p-2 bg-emerald-50 rounded-xl')
                                ui.label('Aggregate Success Rate').classes('text-xs font-bold uppercase text-emerald-800/80 tracking-wider')
                            ...
                            ui.label(f"Performance Level: {status}").classes('text-xs font-semibold text-slate-400 mt-1')

                    with ui.card().classes('w-full p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10 mt-4'):
                        with ui.row().classes('items-center gap-2.5 mb-5'):
                            ui.icon('groups', color='primary').classes('text-xl p-2 bg-emerald-50 rounded-xl')
                            ui.label('Number of Pupils per Class').classes('text-base font-bold text-slate-900')

                        class_counts = dashboard_data.get('class_enrollment', {})

                        class_colors = {
                            'P1': {'bg': 'bg-blue-50/60', 'border': 'border-blue-100', 'text': 'text-blue-800'},
                            'P2': {'bg': 'bg-emerald-50/60', 'border': 'border-emerald-100', 'text': 'text-emerald-800'},
                            'P3': {'bg': 'bg-amber-50/60', 'border': 'border-amber-100', 'text': 'text-amber-800'},
                            'P4': {'bg': 'bg-purple-50/60', 'border': 'border-purple-100', 'text': 'text-purple-800'},
                            'P5': {'bg': 'bg-rose-50/60', 'border': 'border-rose-100', 'text': 'text-rose-800'},
                            'P6': {'bg': 'bg-cyan-50/60', 'border': 'border-cyan-100', 'text': 'text-cyan-800'},
                            'P7': {'bg': 'bg-indigo-50/60', 'border': 'border-indigo-100', 'text': 'text-indigo-800'}
                        }

                        ui.label('Lower Primary (P1 - P3)').classes('text-xs font-bold uppercase text-emerald-800/80 tracking-wider mb-2.5')
                        ...
                        with ui.row().classes('w-full gap-4 flex-wrap mb-6'):
                            for cls_name in lower_classes:
                                ...
                                with ui.card().classes(f"flex-1 min-w-[140px] p-4 {style['bg']} rounded-2xl border {style['border']} items-center justify-center shadow-sm hover:scale-105 transition-transform"):
                                    ui.label(cls_name).classes(f"text-xs font-bold {style['text']} uppercase tracking-wider")
                        ...
                        ui.label('Upper Primary (P4 - P7)').classes('text-xs font-bold uppercase text-emerald-800/80 tracking-wider mb-2.5')
                    ...
                    with ui.row().classes('w-full gap-5 items-stretch justify-center mt-4'):
                        with ui.card().classes('flex-1 p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10'):
                            with ui.row().classes('w-full justify-between items-center mb-5'):
                                ui.label('Subject Averages by Class').classes('text-base font-bold text-slate-900')
                                        ...
                                        for sub, val in class_data.items():
                                            with ui.column().classes('w-full gap-1 mb-1'):
                                                with ui.row().classes('w-full justify-between items-center'):
                                                    ...
                                                    ui.label(f"{val}%").classes('text-xs font-black text-slate-900')
                                                ui.linear_progress(value=val/100 if val > 0 else 0.05, color='primary').classes('w-full rounded-full h-2.5 bg-emerald-50')

                            update_class_averages(class_avg_select.value)

                        with ui.card().classes('flex-1 p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10'):
                            with ui.row().classes('items-center gap-2.5 mb-5'):
                                ui.icon('emoji_events', color='amber-7').classes('text-xl p-2 bg-amber-50 rounded-xl')
                                ui.label('Top Academic Performers').classes('text-base font-bold text-slate-900')

                            with ui.column().classes('w-full gap-3'):
                                top_list = dashboard_data.get('top_students', [])
                                if not top_list:
                                    ui.label('No top student records available.').classes('text-xs text-slate-400 italic py-4')
                                else:
                                    for idx, stud in enumerate(top_list):
                                        with ui.row().classes('w-full items-center justify-between p-3.5 bg-gradient-to-r from-emerald-50/40 to-teal-50/20 rounded-2xl border border-emerald-900/10'):
                                            ...
                                            ui.badge(f"Agg: {stud.get('Aggregates', 0)} | {stud.get('Division', 'N/A')}", color='emerald-50') \
                                                .classes('text-emerald-800 font-bold text-[11px] px-3 py-1.5 rounded-full border border-emerald-200')

                    with ui.card().classes('w-full p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10 mt-4'):
                        with ui.row().classes('items-center justify-between mb-5'):
                            with ui.row().classes('items-center gap-2.5'):
                                ui.icon('history', color='primary').classes('text-xl p-2 bg-emerald-50 rounded-xl')
                                ui.label('Recent System Activity').classes('text-base font-bold text-slate-900')
                            ui.button('View All', on_click=lambda: nav_tabs.set_value(logs_tab)).props('flat dense text-color="primary" size="sm" rounded-pill')
                        ...
                        if not logs_list:
                            ...
                            with ui.column().classes('w-full gap-3'):
                                for log in logs_list[:4]:
                                    with ui.row().classes('w-full justify-between items-center p-3.5 bg-slate-50/70 rounded-2xl border border-emerald-900/10'):
                                        with ui.row().classes('items-center gap-3'):
                                            ui.badge(log.get('username', 'System'), color='emerald-100').classes('text-emerald-900 text-[10px] font-bold px-2.5 py-1 rounded-full')
                                            ui.label(log.get('status', '')).classes('text-xs text-slate-800 font-medium')
                                        ui.label(log.get('timestamp', '')).classes('text-[11px] text-slate-400 font-semibold')

            with ui.tab_panel(teachers_tab).classes('p-0 gap-5 flex flex-col'):
                with ui.card().classes('w-full p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10'):
                    with ui.row().classes('w-full items-center justify-between'):
                        ui.label('Upper Primary Marks Entry Manager').classes('text-lg font-bold text-slate-900')
                        with ui.row().classes('gap-2.5 items-center'):
                            launch_btn = ui.button('Run Upper Insert', icon='add_circle').props('color="primary" unelevated rounded-pill')
                            minimize_btn = ui.button('Minimize', icon='keyboard_arrow_up').props('color="grey-7" flat dense rounded-pill')
                ...
                with ui.card().classes('w-full p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10'):
                    with ui.row().classes('w-full items-center justify-between'):
                        ui.label('Lower Primary Marks Entry Manager (P.1 - P.3)').classes('text-lg font-bold text-slate-900')
                        with ui.row().classes('gap-2.5 items-center'):
                            launch_lower_btn = ui.button('Run Lower Insert', icon='add_circle').props('color="primary" unelevated rounded-pill')
                            minimize_lower_btn = ui.button('Minimize', icon='keyboard_arrow_up').props('color="grey-7" flat dense rounded-pill')
            ...
            with ui.tab_panel(Upper_primary_tab).classes('p-0 gap-5 flex flex-col'):
                with ui.card().classes('w-full p-6 bg-white shadow-md rounded-3xl border border-emerald-900/10 flex flex-col gap-5'):
                    with ui.row().classes('w-full justify-between items-center flex-wrap gap-3'):
                        ui.label('Upper Primary Academic Records').classes('text-lg font-bold text-slate-900')
                        ...
                        with ui.row().classes('items-center gap-2.5 flex-wrap'):
                            ...
                            search_input = ui.input(placeholder='Search student name...').props('dense outlined clearable rounded-pill')
                            ui.button(icon='search', on_click=lambda: apply_filter(search_input.value)).props('dense color="primary" unelevated round')
                            ui.button('Export CSV', icon='download', on_click=export_csv).props('flat dense color="primary" rounded-pill')
                            ui.button('Refresh', icon='refresh', on_click=lambda: refresh_table_data(class_select.value, year_input.value)).props('outline dense text-color="primary" rounded-pill')
                    ...
                    student_table = ui.table(columns=columns, rows=[], row_key='id').classes('w-full shadow-none border border-emerald-900/10 rounded-2xl')
                    
                    with ui.row().classes('w-full justify-start mt-2'):
                        ui.button('Print All', icon='print', on_click=download_all_reports).props('unelevated color="primary" rounded-pill')
                    
                    student_table.add_slot('body-cell-Division', '''
                        <q-td :props="props">
                            <q-badge :color="props.value == 'Div U' ? 'red' : 'green'" class="px-2.5 py-1 rounded-full font-semibold">
                                {{ props.value }}
                            </q-badge>
                        </q-td>
                    ''')
                    ...
                    student_table.add_slot('body-cell-actions', '''
                        <q-td :props="props">
                            <q-btn flat dense round icon="download" color="green-7" @click="$parent.$emit('download_row', props.row)" class="hover:bg-emerald-50">
                                <q-tooltip>Download Report</q-tooltip>
                            </q-btn>
                            <q-btn flat dense round icon="edit" color="blue-7" @click="$parent.$emit('edit_row', props.row)" class="hover:bg-blue-50">
                                <q-tooltip>Edit Record</q-tooltip>
                            </q-btn>
                            <q-btn flat dense round icon="delete" color="red-7" @click="$parent.$emit('delete_row', props.row.id)" class="hover:bg-red-50">
                                <q-tooltip>Delete Record</q-tooltip>
                            </q-btn>
                        </q-td>
                    ''')
            ...
            with ui.tab_panel(lower_primary_tab):
                with ui.card().classes('w-full p-6 shadow-md rounded-3xl border border-emerald-900/10 bg-white'):
            ...
            with ui.tab_panel(logout_tab).classes('p-0 justify-center items-center flex min-h-[45vh]'):
                with ui.card().classes('w-full max-w-sm p-10 text-center items-center flex flex-col gap-6 shadow-2xl rounded-3xl border border-emerald-900/10 bg-white'):
                    ui.icon('power_settings_new', color='#1b4d3e') \
                        .classes('text-6xl bg-emerald-50 p-6 rounded-full shadow-inner')
                    
                    with ui.column().classes('gap-1'):
                        ui.label('Ready to leave?').classes('text-2xl font-black text-slate-900')
                        ui.label('Your session will be securely terminated.').classes('text-sm text-slate-500 font-medium')
                    ...
                    with ui.row().classes('w-full gap-3 mt-2 justify-center'):
                        ...
                        ui.button('Sign Out', icon='logout', on_click=perform_logout) \
                            .props('color="primary" unelevated size="md" rounded-pill') \
                            .classes('px-6 shadow-md')
                        
                        ui.button('Cancel', on_click=lambda: nav_tabs.set_value(starting_tab)) \
                            .props('flat color="grey-7" rounded-pill').classes('px-6')
