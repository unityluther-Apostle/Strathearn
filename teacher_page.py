...
def performance_analytics_hub_content():
  ...
  with content_container:
    ...
    ).classes('text-sm text-stone-500 mb-2')
    ...
    ).classes('w-full md:w-96 bg-white rounded-2xl shadow-sm border border-stone-200')
        ...
        with ui.row().classes('w-full gap-4'):
          ...
          with ui.card().classes('flex-1 p-5 border-l-4 border-l-red-500 bg-white rounded-2xl shadow-sm border-stone-100'):
            ...
            ui.label(data['worst_sub']).classes(
                'text-lg font-bold text-red-800 truncate mt-1'
            )
          ...
          with ui.card().classes(
            ...
            with ui.row().classes('items-center gap-2 mb-4'):
              ...
              ui.label(f'Top Achievers — {class_name}').classes(
                  'text-md font-bold text-emerald-900'
              )
            ...
            if not data['top_students']:
              ...
              for idx, s in enumerate(data['top_students'], 1):
                ...
                with ui.row().classes(
                    'w-full justify-between py-2 border-b border-emerald-100/60'
                    ' items-center'
                ):
                  ...
                  ui.label(f'{round(score, 1)}%').classes(
                      'font-mono font-bold text-sm text-emerald-700'
                  )
          ...
          with ui.card().classes(
            ...
            with ui.row().classes('items-center gap-2 mb-4'):
              ...
              ui.label(f'Academic Intervention — {class_name}').classes(
                  'text-md font-bold text-rose-900'
              )
            ...
            if not data['at_risk']:
              ...
              for s in data['at_risk']:
                ...
                score = s.get('Average') or s.get('_calc_avg') or 0
                with ui.row().classes(
                    'w-full justify-between py-2 border-b border-rose-100/60'
                    ' items-center'
                ):
                  ui.label(name).classes('font-medium text-sm text-stone-800')
                  ui.label(f'{round(score, 1)}%').classes(
                      'font-mono font-bold text-sm text-rose-700'
                  )
...
def staff_chat_content():
  chat_container = ui.column().classes(
      'w-full h-[450px] overflow-y-auto p-4 bg-stone-50/80 rounded-2xl'
      ' shadow-inner border border-stone-200 gap-3'
  )
      ...
      try:
        ...
        if not rows:
          with ui.column().classes(
            ...
            ).classes('text-stone-500 text-xs font-semibold')
            ui.label(
                'Drop a complaint about the printer or start the morning'
                ' gossip.'
            ).classes('text-stone-400 text-[11px] italic')
        else:
          ...
          for r in rows:
            ...
            bubble_bg = (
                'bg-[#1E4D2B] text-white rounded-tr-sm'
                if is_me
                else (
            ...
            with ui.column().classes(f'w-full {align_cls} gap-0.5 my-1'):
              with ui.row().classes('items-center gap-2 px-1'):
                if not is_me:
                  ui.label(sender).classes('text-[11px] font-bold text-[#1E4D2B]')
  ...
  with ui.card().classes(
      'w-full max-w-4xl mx-auto bg-white rounded-3xl shadow-lg border'
      ' border-stone-100 overflow-hidden p-0'
  ):
    # Professional Dark Green & Amber Header with Online Counter & Clear Feed Option
    with ui.row().classes(
        'items-center justify-between w-full bg-[#1E4D2B] px-6 py-4 text-white'
        ' border-b border-emerald-900'
    ):
      with ui.row().classes('items-center gap-3'):
        with ui.avatar(size='sm').classes(
            'bg-amber-500 text-[#1E4D2B] font-extrabold shadow-sm'
        ):
    ...
    with ui.row().classes(
        'w-full gap-1.5 items-center px-4 py-2 bg-stone-50 border-t'
        ' border-stone-200 overflow-x-auto'
    ):
      ui.label('Vibe Check:').classes(
          'text-[10px] text-amber-800 font-bold uppercase mr-1 shrink-0'
      )
      for emo in quick_emojis:
        ...
        ui.button(emo, on_click=insert_emoji).props('flat dense').classes(
            'text-sm bg-white border border-stone-200 rounded-xl px-2.5 py-1'
            ' text-stone-700 hover:bg-amber-50 shrink-0 shadow-sm'
        )

    # Enhanced Communication Input Bar with Character Counter Hint
    with ui.row().classes(
        'w-full items-center gap-3 p-4 bg-white border-t border-stone-200'
    ):
      msg_input = (
      ...
      ui.button(icon='send', on_click=send_message).classes(
          'bg-[#1E4D2B] text-white rounded-2xl p-2.5 h-11 w-11 shadow-md'
...
def teacher():
  ...
  ui.query('.nicegui-content').classes('w-full min-h-screen bg-stone-100/60 p-8')
  ...
  with ui.column().classes('w-full items-center'):
    with ui.column().classes('w-[95%] max-w-7xl gap-8'):
      ...
      with ui.tabs().classes(
          'w-full bg-white/90 backdrop-blur-md p-2 rounded-3xl shadow-md border'
          ' border-stone-200/60'
      ) as tabs:
      ...
      with ui.tab_panels(tabs, value='Dashboard').classes(
        ...
        with ui.tab_panel('Dashboard'):
          ...
          with ui.row().classes('w-full gap-4 mb-6'):
            ...
            with ui.card().classes(
              ...
              ui.label('Primary Section Active').classes(
                  'text-xs text-stone-500 mt-1 font-medium'
              )
            ...
            with ui.card().classes(
              ...
              ui.label(
                  ...
                  f' {lower_count}'
              ).classes('text-xs text-stone-500 mt-1 font-medium')
            ...
            with ui.card().classes(
              ...
              ui.label(str(p7_count)).classes(
                  'text-2xl font-extrabold text-amber-900 mt-1'
              )
          ...
          with ui.row().classes('items-center mb-6 mt-2'):
            ui.icon('analytics', color='indigo', size='2rem')
            ui.label('Performance Analytics').classes(
                'text-2xl font-bold text-stone-900 ml-2'
            )
                  ...
                  with ui.column().classes('flex-1'):
                    ...
                    if not metrics['at_risk']:
                      ...
                      for s in metrics['at_risk']:
                        with ui.row().classes(
                            'w-full justify-between py-1 border-b border-stone-100'
                        ):
                          ...
                          ).classes('font-mono text-xs text-red-600 font-bold')

                  # Column 2: Rising Stars
                  with ui.column().classes(
                      'flex-1 bg-emerald-50/60 p-4 rounded-2xl shadow-sm border'
                      ' border-emerald-100'
                  ):
                    with ui.row().classes('items-center gap-2 mb-3'):
                      ui.icon('workspace_premium', color='green', size='1.5rem')
        ...
        with ui.tab_panel('Logout'):
          with ui.card().classes('w-[350px] mx-auto items-center p-8 rounded-3xl shadow-xl bg-white border border-stone-100'):
            ...
            ).classes('w-full py-3 rounded-2xl font-bold shadow-md')
