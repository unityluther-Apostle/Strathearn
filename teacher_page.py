...
def edit_record(record, on_save):
  with ui.dialog() as dialog, ui.card().classes(
      'w-full max-w-[700px] p-6 md:p-8 bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 rounded-3xl shadow-2xl border border-emerald-100'
  ):
    ...
    ui.button('Save Changes', on_click=save).classes(
        'w-full mt-6 bg-gradient-to-r from-emerald-600 to-teal-700 text-white font-extrabold py-3.5 rounded-2xl'
        ' hover:from-emerald-700 hover:to-teal-800 transition-all shadow-lg shadow-emerald-700/20'
    )
  dialog.open()
...
def edit_lower_record(record, on_save):
  with ui.dialog() as dialog, ui.card().classes(
    ...
    with ui.row().classes('items-center gap-3 mb-4'):
      with ui.avatar(color='emerald-700', text_color='white').props('size=md'):
        ui.icon('edit_note', size='1.5rem')
      ui.label(f"Edit Lower Primary: {record['pupil_name']}").classes(
          'text-xl md:text-2xl font-black text-emerald-950'
      )
    with ui.grid(columns=1).classes('w-full gap-4 md:grid-cols-2'):
      fields = {
          ...
          ).classes('w-full bg-white/80 rounded-xl'),
          'pupil_name': ui.input(
              'Pupil Name', value=record['pupil_name']
          ).classes('w-full bg-white/80 rounded-xl'),
          'class_level': ui.input('Class', value=record['class_level']).classes(
              'w-full bg-white/80 rounded-xl'
          ),
    ...
    def save():
      ...
      ui.notify(
          '✨ Lower primary record updated successfully!',
          color='positive',
          icon='check_circle',
      )
      dialog.close()
      ...
      try:
        ...
        if not raw_rows:
          with ui.card().classes(
              'w-full p-12 items-center bg-white/80 backdrop-blur-md rounded-3xl shadow-sm border border-emerald-100/60'
          ):
            ui.icon('folder_open', size='3rem', color='emerald')
            ui.label('No lower primary records found.').classes(
                'text-emerald-950 font-semibold mt-2'
            )
        else:
          ...
          lower_table = (
              ...
              .classes(
                  'w-full bg-white/90 backdrop-blur-md rounded-3xl shadow-xl'
      ...
      try:
        ...
        if not raw_rows:
          with ui.card().classes(
            ...
            ui.icon('folder_open', size='3rem', color='emerald')
            ui.label('No upper primary records found.').classes(
                'text-emerald-950 font-semibold mt-2'
            )
        else:
...
def performance_analytics_hub_content():
  ...
  with content_container:
    with ui.row().classes('items-center gap-3'):
      with ui.avatar(
          color='emerald-700', text_color='white', size='md'
      ).classes('shadow-md shadow-emerald-700/20'):
        ui.icon('insights', size='1.5rem')
      with ui.column().classes('gap-0'):
        ui.label('Per-Class Performance Hub').classes(
            'text-2xl font-black text-emerald-950'
        )
        ui.label(
            'Select a section and class to view isolated performance metrics,'
            ' top achievers, and intervention lists.'
        ).classes('text-xs text-emerald-800/80 font-medium')
    ...
    selected_dropdown = ui.select(
        ...
        'w-full md:w-96 bg-white/90 backdrop-blur-md rounded-2xl shadow-sm'
    ...
    def update_analytics_view():
      ...
      if not val or 'No Classes Found' in val:
        with metrics_display_area:
          with ui.card().classes(
              'w-full p-8 items-center bg-white/80 rounded-3xl shadow-sm'
              ' border border-emerald-100'
          ):
      ...
      with metrics_display_area:
        if not data:
          with ui.card().classes(
              'w-full p-10 items-center bg-white/90 rounded-3xl shadow-sm'
              ' border border-emerald-100'
          ):
        ...
        # KPI Summary Row with Vibrant Accent Gradients
        with ui.row().classes('w-full gap-4 flex-col md:flex-row'):
          with ui.card().classes(
              'flex-1 p-6 bg-gradient-to-br from-emerald-600 to-teal-800 text-white rounded-3xl shadow-lg shadow-emerald-800/20 relative overflow-hidden'
          ):
            ui.icon('groups', size='2.5rem').classes(
                'absolute right-4 bottom-4 opacity-15'
            )
            ui.label('Total Enrolled').classes(
                'text-xs font-bold text-emerald-200 uppercase tracking-wider'
            )
          ...
          with ui.card().classes(
              'flex-1 p-6 bg-gradient-to-br from-teal-600 to-cyan-800 text-white rounded-3xl shadow-lg shadow-teal-800/20 relative overflow-hidden'
          ):
            ui.icon('percent', size='2.5rem').classes(
                'absolute right-4 bottom-4 opacity-15'
            )
            ui.label('Class Average Score').classes(
                'text-xs font-bold text-teal-200 uppercase tracking-wider'
            )
          ...
          with ui.card().classes(
              'flex-1 p-6 bg-gradient-to-br from-green-600 to-emerald-700 text-white rounded-3xl shadow-lg shadow-green-700/20 relative overflow-hidden'
          ):
            ui.icon('trending_up', size='2.5rem').classes(
                'absolute right-4 bottom-4 opacity-15'
            )
            ui.label('Best Subject').classes(
                'text-xs font-bold text-green-200 uppercase tracking-wider'
            )
          ...
          with ui.card().classes(
              'flex-1 p-6 bg-gradient-to-br from-amber-600 to-orange-700 text-white rounded-3xl shadow-lg shadow-amber-700/20 relative overflow-hidden'
          ):
            ui.icon('trending_down', size='2.5rem').classes(
                'absolute right-4 bottom-4 opacity-15'
            )
            ui.label('Weakest Subject').classes(
                'text-xs font-bold text-amber-200 uppercase tracking-wider'
            )
            ui.label(data['worst_sub']).classes(
                'text-lg font-bold mt-1 truncate'
            )

        # Detailed Breakdown Cards Row (Mobile Responsive)
        with ui.row().classes('w-full gap-6 mt-2 flex-col md:flex-row'):
          # Top Achievers
          with ui.card().classes(
              'flex-1 p-6 bg-gradient-to-br from-emerald-50 via-teal-50/50 to-white border border-emerald-200/80 rounded-3xl shadow-xl'
          ):
            with ui.row().classes('items-center gap-3 mb-4'):
              with ui.avatar(
                  color='emerald-700', text_color='white', size='sm'
              ).classes('shadow-sm'):
                ui.icon('workspace_premium', size='1.2rem')
              ui.label(f'Top Achievers — {class_name}').classes(
                  'text-md font-black text-emerald-950'
              )
              ...
              for idx, s in enumerate(data['top_students'], 1):
                ...
                badge_colors = (
                    ...
                    if idx == 1
                        ...
                        if idx == 2
                        else 'bg-amber-700 text-white font-bold'
                    )
                )
                with ui.row().classes(
                    'w-full justify-between py-2.5 px-3 bg-white/80 rounded-2xl'
                    ' my-1.5 items-center border border-emerald-100 shadow-sm'
                ):
                  with ui.row().classes('items-center gap-3'):
                    ui.badge(str(idx)).classes(
                        f'{badge_colors} px-2 py-0.5 rounded-full text-xs'
                    )
                    ui.label(name).classes(
                        'font-bold text-sm text-emerald-950'
                    )
                  ui.label(f'{round(score, 1)}%').classes(
                      'font-mono font-black text-sm text-emerald-700'
                  )

          # Students Needing Intervention (< 50 average)
          with ui.card().classes(
              'flex-1 p-6 bg-gradient-to-br from-amber-50 via-orange-50/50 to-white border border-amber-200/80 rounded-3xl shadow-xl'
          ):
            with ui.row().classes('items-center gap-3 mb-4'):
              with ui.avatar(
                  color='amber-700', text_color='white', size='sm'
              ).classes('shadow-sm'):
                ui.icon('warning', size='1.2rem')
              ui.label(f'Academic Intervention — {class_name}').classes(
                  'text-md font-black text-amber-950'
              )

            if not data['at_risk']:
              with ui.row().classes(
                  'w-full p-4 bg-white/80 rounded-2xl items-center gap-2 border'
                  ' border-amber-100'
              ):
              ...
              for s in data['at_risk']:
                ...
                with ui.row().classes(
                    'w-full justify-between py-2.5 px-3 bg-white/80 rounded-2xl'
                    ' my-1.5 items-center border border-amber-100 shadow-sm'
                ):
                  ui.label(name).classes('font-bold text-sm text-stone-900')
                  ui.badge(f'{round(score, 1)}%').classes(
                      'bg-amber-100 text-amber-900 font-mono font-bold'
                  )
...
def staff_chat_content():
      ...
      try:
        ...
        if not rows:
          with ui.column().classes(
            ...
            with ui.avatar(
                color='emerald-100', text_color='emerald-800', size='lg'
            ):
              ui.icon('local_cafe', size='1.8rem')
            ui.label(
                'The kettle is boiling, but nobody has spoken yet!'
            ).classes('text-emerald-950 text-sm font-bold mt-2')
            ui.label(
                'Drop a complaint about the printer or start the morning'
                ' gossip.'
            ).classes('text-emerald-800/70 text-xs italic')
        else:
          ...
          for r in rows:
            ...
            bubble_bg = (
                ...
                ' rounded-tr-sm shadow-md shadow-emerald-700/10'
                if is_me
                else (
                    'bg-white text-stone-800 rounded-tl-sm border'
              ...
              with ui.row().classes('items-center gap-2 px-1'):
                ...
                if is_me:
                  ui.button(
                      ...
                      'text-stone-400 hover:text-red-600 text-[10px] p-0 min-h-0'
                      ' min-w-0'
                  )

              with ui.card().classes(
                  f'{bubble_bg} p-3.5 rounded-2xl max-w-[85%] md:max-w-[75%]'
              ):
                ui.label(r['message']).classes(
                    'text-sm whitespace-pre-wrap leading-relaxed break-words'
                )
                with ui.row().classes(
                    'w-full justify-end items-center gap-1 mt-1'
                ):
                  ui.label(r['timestamp']).classes(
                      'text-[9px] text-emerald-200'
                      if is_me
                      else 'text-stone-400'
                  )
                  if is_me:
  ...
  with ui.column().classes(
      'w-full max-w-3xl mx-auto bg-white/90 backdrop-blur-md rounded-3xl shadow-xl'
      ' border border-emerald-100 overflow-hidden'
  ):
    # Professional Vibrant Dark Green Header with Online Counter & Clear Feed Option
    with ui.row().classes(
        'items-center justify-between w-full bg-gradient-to-r from-emerald-800'
        ' via-teal-800 to-cyan-900 px-5 py-4 text-white border-b'
        ' border-emerald-950'
    ):
      with ui.row().classes('items-center gap-3'):
        with ui.avatar(size='md').classes(
            'bg-white/20 text-white font-extrabold shadow-sm backdrop-blur-sm'
        ):
        ...
        with ui.column().classes('gap-0'):
          ...
          ui.label(current_vibe).classes(
              'text-[11px] text-emerald-200 font-medium'
          )

      with ui.row().classes('items-center gap-1'):
        ui.badge('Live Sync', color='emerald-700').props('dense').classes(
    ...
    with ui.row().classes(
        'w-full gap-1 items-center px-4 py-2 bg-emerald-50/50 border-t'
        ' border-emerald-100 overflow-x-auto'
    ):
      ui.label('Vibe Check:').classes(
          'text-[10px] text-emerald-800 font-black uppercase mr-1 shrink-0'
      )
      for emo in quick_emojis:
        ...
        ui.button(emo, on_click=insert_emoji).props('flat dense').classes(
            'text-sm bg-white border border-emerald-200 rounded-xl px-2 py-0.5'
            ' text-stone-700 hover:bg-emerald-100 shrink-0 shadow-sm'
        )

    # Enhanced Communication Input Bar
    with ui.row().classes(
        'w-full items-center gap-2 p-3.5 bg-white border-t border-emerald-100'
    ):
      msg_input = (
          ui.input(placeholder='Say something before the bell rings...')
          .classes(
              'flex-1 bg-stone-100 text-stone-900 rounded-2xl px-4 border-none'
          )
...
def teacher():
  ...
  ui.query('.nicegui-content').classes(
      'w-full min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50/40'
      ' to-cyan-50/50 p-4 md:p-8'
  )
  ...
  with ui.column().classes('w-full items-center'):
    with ui.column().classes('w-[98%] md:w-[95%] max-w-7xl gap-6 md:gap-8'):
      # --- Header with Vibrant Gradient & Icon ---
      with ui.card().classes(
          'w-full p-8 md:p-12 bg-gradient-to-r from-emerald-900 via-teal-900 to-cyan-950 shadow-xl rounded-3xl text-center border border-emerald-700/30 relative overflow-hidden'
      ):
        ui.icon('school', size='6rem', color='emerald').classes(
            'absolute -right-8 -bottom-8 opacity-10'
        )
        ...
        ui.label(
            'School Report & Academic Management System'
        ).classes('text-emerald-200 text-sm md:text-base font-medium mt-1')

      # --- Daily Inspiration ---
      with ui.card().classes(
          'w-full p-6 bg-gradient-to-r from-emerald-100/80 via-teal-50 to-white border-l-8 border-emerald-600 rounded-3xl shadow-sm border border-emerald-100'
      ):
        with ui.row().classes('items-center gap-2 mb-1'):
          ui.icon('menu_book', color='emerald-800', size='1.2rem')
          ui.label('📖 Daily Inspiration').classes(
              'text-emerald-900 font-extrabold text-xs tracking-wider uppercase'
          )
        ui.label(random.choice(verses)).classes(
            'text-emerald-950 italic text-lg md:text-xl font-medium'
        )

      # --- Tabs with Enhanced Active States ---
      with ui.tabs().classes(
          'w-full bg-white/90 backdrop-blur-md p-2 rounded-3xl shadow-lg shadow-emerald-950/5 border'
          ' border-emerald-100 overflow-x-auto flex-nowrap'
      ) as tabs:
        tabs.classes('items-center justify-start md:justify-center')
        ui.tab('Dashboard', icon='dashboard').classes(
            'rounded-2xl transition-all duration-300 hover:bg-emerald-50 text-emerald-900 font-bold'
        )
        ...
        ui.tab('Logout', icon='logout').classes(
            'rounded-2xl transition-all duration-300 hover:bg-red-50 text-red-600 font-bold'
        )
      ...
      with ui.tab_panels(tabs, value='Dashboard').classes(
        ...
        with ui.tab_panel('Dashboard'):
          ...
          # --- Termly Context & Overview Cards (Responsive Column/Row with Vibrant Gradients) ---
          with ui.row().classes('w-full gap-4 mb-6 flex-col md:flex-row'):
            # Active Term Card
            with ui.card().classes(
                'flex-1 p-6 bg-gradient-to-br from-emerald-800 to-teal-900 text-white shadow-xl shadow-emerald-900/15 rounded-3xl relative overflow-hidden'
            ):
              ui.icon('calendar_month', size='2.5rem').classes(
                  'absolute right-4 bottom-4 opacity-15'
              )
              ...
              ui.label('Primary Section Active').classes(
                  'text-xs text-emerald-100/80 mt-1 font-medium'
              )

            # Total Enrolled Pupils Card
            try:
              with sqlite3.connect(DB) as conn:
            ...
            with ui.card().classes(
                'flex-1 p-6 bg-gradient-to-br from-teal-700 to-cyan-900 text-white shadow-xl shadow-teal-900/15 rounded-3xl relative overflow-hidden'
            ):
              ui.icon('badge', size='2.5rem').classes(
                  'absolute right-4 bottom-4 opacity-15'
              )
              ui.label('TOTAL ENROLLED').classes(
                  'text-xs font-bold text-teal-200 uppercase tracking-wider'
              ...
              ui.label(
                  ...
                  f' {lower_count}'
              ).classes('text-xs text-teal-100/80 mt-1 font-medium')
            ...
            with ui.card().classes(
                'flex-1 p-6 bg-gradient-to-br from-green-700 to-emerald-900 text-white shadow-xl shadow-green-900/15 rounded-3xl relative overflow-hidden'
            ):
              ui.icon('military_tech', size='2.5rem').classes(
                  'absolute right-4 bottom-4 opacity-15'
              )
              ...
              ui.label('Registered for PLE Assessment').classes(
                  'text-xs text-green-100/80 mt-1 font-medium'
              )
          ...
          if metrics:
            with ui.row().classes('w-full gap-6 flex-col md:flex-row'):
              # KPIs
              with ui.card().classes(
                  'w-full md:w-1/3 p-6 shadow-xl bg-white/90 backdrop-blur-md rounded-3xl border border-emerald-100'
              ):
                ...
                with ui.row().classes('w-full justify-around mt-2'):
                  with ui.column().classes(
                      'items-center p-4 bg-emerald-50 rounded-2xl flex-1 mr-2 border'
                      ' border-emerald-100'
                  ):
                    ui.icon('arrow_upward', color='green', size='1.8rem')
                    ui.label('Top').classes(
                        'text-xs text-stone-400 font-bold uppercase mt-1'
                    )
                    ui.label(metrics['best']).classes(
                        'text-lg font-black text-green-700'
                    )
                  with ui.column().classes(
                      'items-center p-4 bg-amber-50 rounded-2xl flex-1 ml-2 border'
                      ' border-amber-100'
                  ):
                    ...
                    ui.label(metrics['worst']).classes(
                        'text-lg font-black text-amber-700'
                    )

              # Support & Achievement Lists
              with ui.card().classes(
                  'w-full md:w-[63%] p-6 shadow-xl bg-white/90 backdrop-blur-md rounded-3xl border border-emerald-100'
              ):
                with ui.row().classes('w-full gap-8 flex-col md:flex-row'):
                  # Column 1: Academic Intervention (< 50 average)
                  with ui.column().classes(
                      'flex-1 bg-amber-50/50 p-4 rounded-2xl border'
                      ' border-amber-100'
                  ):
                    with ui.row().classes('items-center gap-2 mb-3'):
                      with ui.avatar(
                          color='amber-700', text_color='white', size='xs'
                      ):
                    ...
                    if not metrics['at_risk']:
                      ...
                      ).classes('text-xs text-stone-400 italic')
                    else:
                      for s in metrics['at_risk']:
                        with ui.row().classes(
                            'w-full justify-between py-1.5 px-2 bg-white rounded-xl'
                            ' my-1 border border-amber-100 shadow-sm items-center'
                        ):
                          ui.label(f"{s['Name']} ({s['Class']})").classes(
                              'text-xs font-bold text-stone-800'
                          )
                          ui.badge(f"{round(s['Average'] or 0, 1)}%").classes(
                              'bg-amber-100 text-amber-900 font-mono font-bold'
                          )

                  # Column 2: Rising Stars
                  with ui.column().classes(
                      'flex-1 bg-emerald-50/70 p-4 rounded-2xl border'
                      ' border-emerald-200'
                  ):
                    with ui.row().classes('items-center gap-2 mb-3'):
                      with ui.avatar(
                          color='emerald-700', text_color='white', size='xs'
                      ):
                    ...
                    if not metrics['rising']:
                      ui.label('No rising stars identified yet.').classes(
                          'text-xs text-stone-400 italic'
                      )
                    else:
                      for s in metrics['rising']:
                        with ui.row().classes(
                            'w-full justify-between py-1.5 px-2 bg-white rounded-xl'
                            ' my-1 border border-emerald-100 shadow-sm items-center'
                        ):
                          ui.label(f"{s['Name']} ({s['Class']})").classes(
                              'text-xs font-bold text-stone-800'
                          )
                          ui.badge(f"{round(s['Average'] or 0, 1)}%").classes(
                              'bg-emerald-100 text-emerald-900 font-mono font-bold'
                          )
          else:
            with ui.card().classes(
                'w-full p-12 items-center bg-white/90 rounded-3xl shadow-xl border border-emerald-100'
            ):
              ui.icon('analytics', size='3rem', color='emerald')
              ui.label('No academic data available for analysis.').classes(
                  'text-emerald-950 font-bold mt-2'
              )

        # --- Class Analytics Panel ---
        with ui.tab_panel('Class Analytics'):
          with ui.card().classes(
              'w-full p-6 bg-white/90 backdrop-blur-md rounded-3xl shadow-xl border border-emerald-100'
          ):
        ...
        with ui.tab_panel('Logout'):
          with ui.card().classes(
              'w-full max-w-[320px] mx-auto items-center p-8 bg-white/90 backdrop-blur-md rounded-3xl shadow-xl border border-emerald-100'
          ):
            with ui.avatar(
                color='red-100', text_color='red-600', size='xl'
            ).classes('shadow-sm'):
              ui.icon('logout', size='2.5rem')
            ui.label('Ready to leave?').classes(
                'text-xl font-black mt-4 text-stone-900'
            )
            ui.label('Your session will be securely closed.').classes(
                'text-stone-500 mb-6 text-xs text-center'
            )
