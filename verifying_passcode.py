...
Table = {
    "Users": '''Username TEXT UNIQUE, Password BLOB, Email TEXT UNIQUE, current_session_id TEXT, is_active INTEGER DEFAULT 1'''
}
...
def add_user(username, plain_password, email):
    ...
    try:
        cursor.execute(
            "INSERT INTO Users (Username, Password, Email, is_active) VALUES(?,?,?, 1);", (username, hashed_password, email)
        )
        conn.commit()
...
def verify_user(username, plain_password):
    ...
    try:
        ...
        stored_hash, is_active = row[0], row[1]
        
        if is_active == 0:
            print(f"Verification Failed: User '{username}' account is inactive or has been logged out.")
            return False
